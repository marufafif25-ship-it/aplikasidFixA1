...
import { fetchChatFaq, fetchFooterSettings, fetchHomepageSettings, fetchProducts } from "../lib/products-api";
...
const mergeStoredOrder = (items) => {
  ...
  try {
    ...
    const remoteHasOrder = items.some((product) => product.sortOrder !== "" && product.sortOrder !== null && product.sortOrder !== undefined && Number.isFinite(Number(product.sortOrder)));
    return orderProducts(items.map((product, index) => ({ ...product, sortOrder: remoteHasOrder ? product.sortOrder : (savedOrder.has(product.id) ? savedOrder.get(product.id) : index) })));
  } catch {
    return items;
  }
};
